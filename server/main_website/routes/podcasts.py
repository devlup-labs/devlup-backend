...
async def create_podcast(
    ...
    main_db_sync.podcasts.insert_one(podcast.model_dump())
...
def get_podcasts():
  ...
  #  REMOVE podcast_url from exclusion
    podcasts = list(main_db_sync.podcasts.find({}, {"_id": 0}))
    return {
        "success": True,
        "data": podcasts,
...
def get_podcast(podcast_id: str):
    podcast = main_db_sync.podcasts.find_one({"podcast_id": podcast_id}, {"_id": 0})
    if not podcast:
...
def update_podcast(
    ...
    existing_podcast = main_db_sync.podcasts.find_one(
    ...
    main_db_sync.podcasts.update_one(
        {"podcast_id": podcast_id},
        {"$set": update_data}
    )

    updated_podcast = main_db_sync.podcasts.find_one(
        {"podcast_id": podcast_id},
        {"_id": 0}
    )
...
def delete_podcast(podcast_id: str):
    result = main_db_sync.podcasts.delete_one({"podcast_id": podcast_id})
